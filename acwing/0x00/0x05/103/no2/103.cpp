...
using namespace std;
const int N=2e5+5;
int n,m,people[N],video[N],caption[N],all,language[3*N],tot[3*N];
int find(int x){
    return lower_bound(language+1,language+n+1,x)-language;
}
void solve(){
    cin>>n;
    for(int i=1;i<=n;i++){
    ...
    int length=unique(language+1,language+n+1)-(language+1);
